...
gltfLoader.load("/models/human/human.gltf", (gltf) => {
	model = gltf.scene;
		...
		scrollTrigger: {
			...
			onUpdate: (self) => {
				...
				for (let i = 0; i < models.length; i++) {
					...
					models[i].position.x = (Math.cos(radians[i]) * speed[i] * self.progress) / 2;
					models[i].position.z = (Math.sin(radians[i]) * speed[i] * self.progress) / 2;
				}
			},
		},
	});
